...
=========================================
- /taggroup [message]  — mention all group members in batches of 4
                         with a mandatory 3-second delay between batches
- /tagall [message]    — alias for /taggroup
- /stoptag             — cancel active tagging
- /tagstatus           — show current tagging progress

Rules:
  - Only group admins can trigger tagging.
  - BATCH_SIZE = 4 (exactly 4 mentions per message).
  - BATCH_DELAY = 3.0 seconds between every batch (anti-ban).
  - Visible mentions ONLY — no invisible/zero-width character tricks.
"""
...
BATCH_SIZE      = 4      # 4 mentions per message — TOS compliant
BATCH_DELAY     = 3.0    # mandatory 3-second anti-ban delay between batches
COOLDOWN_SECONDS = 300   # 5-minute cooldown per group between runs
...
def _create_visible_mention(user) -> str:
    """Visible mention using the user's real display name. No invisible tricks."""
...
async def execute_taggroup(
    ...
    return (
        True,
        f"Starting to tag {len(real_members)} members...\n"
        f"Batch size: {BATCH_SIZE} | Delay: {int(BATCH_DELAY)}s between batches",
    )
...
async def taggroup_cmd(client: Client, message: Message):
    """Tag all group members in visible batches of 4. Admins only."""
    ...
    success, status_msg = await execute_taggroup(client, chat_id, admin_id, custom_message=custom)
