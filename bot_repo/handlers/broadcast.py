...
Command: /broadcast with sub-options:
  - all      : Broadcast to everyone (users + groups)
  - private  : Only to individual users
  - groups   : Only to groups

Safety: 0.2-second delay between every single message.
"""
...
BROADCAST_DELAY = 0.2  # 0.2 seconds between each message (anti-spam)
...
async def broadcast_start(client: Client, message: Message):
    """
    /broadcast — Multi-level broadcast with sub-options.
    Usage:
    ...
    if args and args[0].lower() in ("all", "private", "groups"):
        # Direct sub-option shortcut
        ...
        await message.reply_text(
            ...
            f"Delay: {BROADCAST_DELAY}s between each message\n\n"
            f"Send the message you want to broadcast (text, photo, video, file).\n"
            f"Type /cancel to cancel.",
            parse_mode=HTML,
        )
        return

    # Show menu if no sub-option given
    await message.reply_text(
...
async def sbc_start(client: Client, message: Message):
    ...
    await message.reply_text(
        "<b>Scheduled Broadcast</b>\n\nChoose who to send this broadcast to:\n\n"
        "Type /cancel to cancel.",
        parse_mode=HTML,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("All (Users + Groups)", callback_data="bc_all")],
